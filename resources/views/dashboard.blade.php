        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div class="bg-blue-100 rounded-lg shadow p-6 flex flex-col items-center">
                    <span x-data="{ count: 0, target: {{ $summaryData['total_cases'] ?? 0 }} }" x-init="let i = setInterval(() => { if(count < target) count++; else clearInterval(i); }, 10)" class="text-3xl font-bold text-blue-900" x-text="count"></span>
                    <span class="mt-2 text-blue-700">Total Cases</span>
                </div>
                <div class="bg-yellow-100 rounded-lg shadow p-6 flex flex-col items-center">
                    <span x-data="{ count: 0, target: {{ $summaryData['current_issues'] ?? 0 }} }" x-init="let i = setInterval(() => { if(count < target) count++; else clearInterval(i); }, 30)" class="text-3xl font-bold text-yellow-900" x-text="count"></span>
                ...
                <div class="bg-green-100 rounded-lg shadow p-6 flex flex-col items-center">
                    <span x-data="{ count: 0, target: {{ $summaryData['completed_issues'] ?? 0 }} }" x-init="let i = setInterval(() => { if(count < target) count++; else clearInterval(i); }, 10)" class="text-3xl font-bold text-green-900" x-text="count"></span>
                ...
                <div class="bg-indigo-100 rounded-lg shadow p-6 flex flex-col items-center">
                    <span x-data="{ count: 0, target: {{ $summaryData['responders'] ?? 0 }} }" x-init="let i = setInterval(() => { if(count < target) count++; else clearInterval(i); }, 80)" class="text-3xl font-bold text-indigo-900" x-text="count"></span>
